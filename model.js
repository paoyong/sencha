...
function getComments(userId, postId, callback) {
    ...
    if (userId) {
        ...
        query = 'SELECT * FROM (SELECT DISTINCT ON (c.id) c.id, author, message, score, age, path, depth, (CASE WHEN user_id = ' + userId + ' THEN true ELSE false END) as upvoted FROM (' + commentThreadQuery + ') c LEFT OUTER JOIN comment_upvoted ON c.id = comment_id ORDER BY c.id, upvoted DESC) as q ORDER BY path';
    }
